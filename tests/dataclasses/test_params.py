...
def test_parameters():
    ...
    assert p[8].name == "attributes"
    assert ast.unparse(p[8].type) == "list[Attribute]"
    assert p[9].name == "classes"
    assert ast.unparse(p[9].type) == "list[Class]"
    assert p[10].name == "functions"
    assert ast.unparse(p[10].type) == "list[Function]"
    assert p[11].name == "bases"
    assert ast.unparse(p[11].type) == "list[Class]"
